  ...
  return (
    <div ref={ref} className="bg-white text-black p-4 text-xs font-mono" style={{ width: '302px' }}>
        ...
        <tbody>
          {cart.map((item) => (
            <tr key={item.product.id}>
              <td className="py-0.5 pr-1 truncate max-w-[100px]">{item.product.name}</td>
              <td className="text-center py-0.5">{item.quantity}</td>
              <td className="text-right py-0.5">₹{item.product.discountPrice}</td>
              <td className="text-right py-0.5">₹{(item.product.discountPrice * item.quantity).toFixed(0)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="flex flex-col items-center gap-2 my-3">
        <QRCodeSVG value={`INV:${invoiceNumber}|AMT:${grandTotal}|DATE:${dayjs().format('YYYYMMDD')}`} size={80} />
        <div className="transform scale-75 origin-center">
          <Barcode value={invoiceNumber} width={1.2} height={30} fontSize={8} displayValue={false} />
        </div>
      </div>
