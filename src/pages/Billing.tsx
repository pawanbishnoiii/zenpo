  ...
  return (
    <div className="h-screen flex flex-col lg:flex-row">
      <div className={`flex-1 flex flex-col ${showCart ? 'hidden lg:flex' : 'flex'}`}>
        <div className="px-4 pt-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <motion.h1 initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-2xl font-bold font-display text-foreground">Billing</motion.h1>
            </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-4">
          {products.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
              ...
              <p className="text-xs mt-1">Go to Workspace to add products first</p>
        ...
        {cart.length > 0 && (
          <motion.button initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} onClick={() => setShowCart(true)}
            ...
            View Cart ({cart.length} items) — ₹{cart.reduce((s, i) => s + i.product.discountPrice * i.quantity, 0).toFixed(0)}
          </motion.button>
        )}
      </div>
      <div className={`lg:w-96 lg:border-l border-border bg-card flex flex-col ${showCart ? 'fixed inset-0 z-50 lg:relative' : 'hidden lg:flex'}`}>
        {showCart && (
