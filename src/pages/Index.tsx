...
const features = [
  { icon: ScanLine, title: 'Barcode Scanner', desc: 'Instant scan and bill with camera' },
  { icon: Printer, title: 'Ezo Printer', desc: 'Bluetooth thermal receipt printing' },
  { icon: BarChart3, title: 'Smart Reports', desc: 'Revenue analytics and insights' },
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-8 space-y-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4 pt-8">
          ...
          <p className="text-muted-foreground text-sm md:text-base max-w-md mx-auto">
            Smart billing, inventory and customer management for Car Wash Centers and Spare Parts shops
          </p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="grid grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl glass-card shadow-soft p-5 space-y-3 border-2 border-accent/10">
            ...
            <h3 className="text-sm font-bold text-foreground">Spare Parts and Mods</h3>
            ...
            {features.map((f, i) => {
              ...
              return (
                <motion.div key={f.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.05 }}
                  ...
                  <div className="w-10 h-10 mx-auto rounded-xl bg-primary/10 flex items-center justify-center"><Icon className="w-5 h-5 text-primary" /></div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="space-y-4">
          <div className="rounded-2xl glass-card shadow-soft p-6 text-center space-y-3">
            ...
            <p className="text-sm text-foreground italic">"ZEN POS completely transformed our car wash billing system. Easy to use and incredibly fast!"</p>
